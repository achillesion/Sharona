...
const Mint = (props) => {
  ...
  return (
    <Plane name='Right_Image' ref={ref} args={[1.15, 2.2]} onPointerEnter={() => Hovered()} onPointerLeave={() => UnHovered()} toneMapped={false} position={[3.1, -0.15, 0.5]} rotation={[0, -Math.PI * 0.45, 0]} scale={[3, 1.2, 1]} transparent opacity={1}>
      <meshPhysicalMaterial side={THREE.DoubleSide} color={'#ddd'} map={texture} />
    </Plane>
  )
}
...
//     <Plane name='Right_Image' ref={ref} args={[1.15, 2.2]} onPointerEnter={() => Hovered()} onPointerLeave={() => UnHovered()} toneMapped={false} position={[3.5, -0.15, 0.5]} rotation={[0, -Math.PI * 0.45, 0]} scale={[4.5, 2.8, 1]} url={props.url} tranparent opacity={1}>
